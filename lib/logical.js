...
function logicalTree(tree) {
  ...
  var rootDir = path.dirname(tree.__filename);
  var devDeps = [Promise.resolve()];
  if (tree.__devDependencies) {
    devDeps = Object.keys(tree.__devDependencies).map(function (name) {
  ...
  return Promise.all(devDeps).then(function (res) {
    ...
    walk(tree.dependencies, function (dep, name, tree) {
      if (!dep.__used) {
        if (potentialDevDeps[name]) {
          if (semver.satisfies(dep.version, potentialDevDeps[name])) {
            delete tree[name];
            return;
          }
        }
