...
function logicalTree(fileTree: PackageExpanded, options: Options) {
  ...
  walk(fileTree.dependencies, function (dep) {
    ...
    if (!dep.__used) {
      ...
      walk(leaf.dependencies, function (notFoundInPackageJsonDep) {
        notFoundInPackageJsonDep.extraneous = true;
        notFoundInPackageJsonDep.depType = depTypes.EXTRANEOUS;
      });
...
function insertLeaf(tree, leaf, from) {
  ...
  const pathToPackage = (from || []).slice(1, -1).map(function (pkg) {
    return moduleToObject(pkg).name;
  });
  let entry = tree.dependencies;
  for (let i = 0; i < pathToPackage.length; i++) {
    if (entry[pathToPackage[i]]) {
      entry = entry[pathToPackage[i]].dependencies;
    }
  }
